...
class TestDatasetImporterFactory(unittest.TestCase):
    ...
    def test_dsc_file_without_extension_returns_tez_importer(self):
        source = os.path.join(
            ROOTPATH, "testdata", "BES3T", "pentacen-transient-field-vs-time"
        )
        importer = self.factory.get_importer(source=source)
        self.assertIsInstance(importer, trepr.io.BES3TImporter)
...
class TestTezImporter(unittest.TestCase):
    ...
    def test_infofile_is_imported(self):
        ...
        self.assertEqual(
            0.6324555320336759, self.dataset.metadata.bridge.power.value
        )
...
class TestFsc2Importer(unittest.TestCase):
    ...
    def test_with_minimal_fsc2_file_and_comment(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            comment, self.dataset.annotations[0].annotation.content
        )
    ...
    def test_import_assigns_recorder_metadata(self):
        ...
        self.assertEqual(
            "mV", self.dataset.metadata.recorder.sensitivity.unit
        )
        ...
        self.assertEqual(
            1e-6, self.dataset.metadata.recorder.pretrigger.value
        )
    ...
    def test_import_assigns_bridge_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            9.75, self.dataset.metadata.bridge.mw_frequency.value
        )
        self.assertEqual(
            "GHz", self.dataset.metadata.bridge.mw_frequency.unit
        )
    ...
    def test_import_assigns_temperature_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            299.0, self.dataset.metadata.temperature_control.temperature.value
        )
        self.assertEqual(
    ...
    def test_import_assigns_pump_metadata(self):
        self.prepare_fsc2_file(
            ...
            header=[
                ...
                "End field           = 4300.00000 G",
                "MW frequency        = 9.75000000 GHz",
                "Laser wavelength = 430.000000 nm (" "10.0000000 Hz)",
                "Number of points    = 10",
                "Trigger position    = 1",
                "Slice length        = 10.0000000 us",
                f"Number of points   = 10",
        ...
        self.importer.source = self.datafile
        self.dataset.import_from(self.importer)
        self.assertEqual(430.0, self.dataset.metadata.pump.wavelength.value)
        self.assertEqual("nm", self.dataset.metadata.pump.wavelength.unit)
        self.assertEqual(
            10.0, self.dataset.metadata.pump.repetition_rate.value
        )
        self.assertEqual(
            "Hz", self.dataset.metadata.pump.repetition_rate.unit
        )
    ...
    def test_import_assigns_experiment_srt_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            10.0, self.dataset.metadata.experiment.shot_repetition_rate.value
        )
        self.assertEqual(
    ...
    def test_import_assigns_recorder_model_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            "Tektronix TDS520A", self.dataset.metadata.recorder.model
        )
    ...
    def test_import_assigns_magnetic_field_devices_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            "Bruker BH15", self.dataset.metadata.magnetic_field.controller
        )
        self.assertEqual(
            "Bruker BH15", self.dataset.metadata.magnetic_field.power_supply
        )
        self.assertEqual(
            "Bruker BH15",
    ...
    def test_import_assigns_aeg_magnetic_field_devices_metadata(self):
        ...
        self.dataset.import_from(self.importer)
        self.assertEqual(
            "home-built", self.dataset.metadata.magnetic_field.controller
        )
        self.assertEqual(
            "AEG Magnet Power Supply",
...
class TestBES3TImporter(unittest.TestCase):
    ...
    def test_with_minimal_1D_file(self):
        ...
        self.assertAlmostEqual(
            9.722e9, self.dataset.metadata.bridge.mw_frequency.value
        )
        self.assertAlmostEqual(
            0.0006454, self.dataset.metadata.bridge.power.value
        )
    ...
    def test_2d_data_with_time_axis_first_assigns_axes(self):
        ...
        self.assertListEqual(
            [320.0, 340.0], list(self.dataset.data.axes[0].values)
        )
        self.assertListEqual(
            [0, 2.5e-6, 5.0e-6, 7.5e-6, 10.0e-6],
